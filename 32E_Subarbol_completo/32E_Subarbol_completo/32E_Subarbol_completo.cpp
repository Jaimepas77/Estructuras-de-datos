...
std::pair <int, int> resolver(bintree <char> tree) {
    ...
    else
    {
        ...
        int record = std::max(izq.first, dcha.first);
        int profAct = std::min(izq.second, dcha.second)+1;
        record = std::max(record, profAct);

        return { record, profAct };
    }
}
...
void resuelveCaso() {
    ...
    std::pair <int, int> sol = resolver(tree);//first - record; second - prof completa actual
    // escribir sol
    std::cout << sol.first << '\n';
}
