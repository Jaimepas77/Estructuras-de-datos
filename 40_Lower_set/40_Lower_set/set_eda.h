...
protected:
    ...
    std::pair<bool, int> limite_inferior(T const& e, const Link& a) const {
        if (a == nullptr)//Si llegamos al final de una rama...
            return { false, 0 };
        else if (menor(e, a->elem)) {
            std::pair<bool, int> aux = limite_inferior(e, a->iz);
            if (aux.first)//Si hay alguno menor...
                return aux;
            else//Si no...
                return { true, a->elem };
        }
        else if (menor(a->elem, e)) {
            return limite_inferior(e, a->dr);
        }
        else // el elemento e ya está en el árbol
            return { true, a->elem };
    }
